...
@st.cache_data
def _high_res_jpeg_bytes(text, line_width, n_shift, shift_range, seed):
    source = render(
        text,
        size=3508,
...
size = st.slider("size", 128, 1024, DEFAULT_PARAMS["size"], step=64, key="size")
n_shift = st.slider("n_shift", 0, 30, DEFAULT_PARAMS["n_shift"], key="n_shift")
shift_range = st.slider("shift_range", 0.0, 0.05, DEFAULT_PARAMS["shift_range"], step=0.001, format="%.3f", key="shift_range")
line_width = st.slider("line width", 0.5, 6.0, DEFAULT_PARAMS["line_width"], step=0.5, key="line_width")
...
st.download_button(
    label="Download A4 300dpi JPEG",
    data=_high_res_jpeg_bytes(text, line_width, n_shift, shift_range, seed),
